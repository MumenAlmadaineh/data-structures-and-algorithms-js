...
Write a function named screenForNames that takes in an array of items and uses Regex to create a new array of only those items that match the following rules:
...
const screenForNames = (arr) => {
  ...
  let vlaueOfVal = /^([MD]r|[M]rs|Ms).\s[A-Z]/g;
  let arrayOfRes = [];
  arr.map(item => {
    vlaueOfVal.test(item) && arrayOfRes.push(item);
  });
  return arrayOfRes;
};
...
Write a function named toTitleCase that takes in an array of items and returns an array of items with the first character in upper case and the rest as is.
...
The names should be combined into a single item with each character name separated by a dash.
...
let biggerThanLuke = (arr) => {
  ...
  let namesOfCha = arr.filter((item) => {
    if (item.mass > 77)
      return item;
  }).map((item) => {
    return item.name;
...
Write a function named detectTicTacToeWin that accepts a two-dimensional array of items. Each item is guaranteed to be either "X", "O" or an empty item. Your function should check to see if any row, column, or either diagonal direction has three matching "X" or "O" symbols (non-empty items), three-in-a-line.
